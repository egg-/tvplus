...
Content.prototype.loadRequest = function (type, opts, cb) {
  ...
  return this.api.request('/monitor/v1/content/' + type, param, function (err, data) {
    ...
    if (type === TYPES.INTILE) {
      result.items = data.in_tile
    } else {
 ...
 * @param  {function} cb cb(err, data)
 */
Content.prototype.loadIntiles = function (opts, cb) {
  return this.loadRequest(TYPES.INTILE, opts, cb)
}
 ...
 */
Content.prototype.updateIntiles = function (items, opts, cb) {
  return this.updateRequest(TYPES.INTILE, items, opts, cb)
}
